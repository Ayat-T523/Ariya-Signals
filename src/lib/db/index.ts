...
// ── USER STATE (Frontend Step 3.5) ────────────────────────────────────────────
// user_profiles / watched_assets / read_alerts / alert_handling_state query
// functions lived here previously. Removed: this state is now local-only
// (see AppContext.tsx), and Supabase is no longer the target architecture for
// per-user state at all -- see this step's checkpoint. HandlingState is kept
// (still the type for the local handlingStates record); the DB-writing
// functions and DbUserProfile are not, since nothing calls them anymore.
