 ...
 * remove one. Measured on live data: 46 terms, 155 of 287 signals, the extra two
 * being NTLA-2002 publications reachable only via the DB synonym.
 *
 ...
 * what the ingest resolver matches on (scripts/lib/signal-gate.mjs,
 * loadAssetResolver). Frontend relevance matching and ingest attribution must
 * draw on the same terms, or a signal can be attributed to an asset at ingest
 * and then judged irrelevant to that same asset in the UI.
 */
