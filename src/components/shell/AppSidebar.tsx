 ...
 * `size-6!` (important) is required here, not decorative: SidebarMenuButton's
 * own CVA class carries `[&_svg]:size-4` (a descendant-selector rule sizing
 * every menu-item ICON to 16px), which has higher specificity than a plain
 * class on this svg and was silently shrinking the 28px brand mark down to
 * 16px -- forced back to size-6 (24px, matching the single-line "Ariya
 * Signals" text height beside it) rather than removed at the source, since
 * every OTHER icon in this button-variant genuinely does want size-4.
 */
function BrandMark() {
  return (
    <svg viewBox="17 -1 50 50" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden className="size-6! shrink-0">
  ...
  return (
    ...
    <Sidebar variant="sidebar" collapsible="icon" {...props}>
      <SidebarHeader>
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton onClick={() => navigate('/')} className="cursor-pointer gap-2.5 text-sidebar-foreground">
              <BrandMark />
