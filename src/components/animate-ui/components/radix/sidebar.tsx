...
type SidebarProviderProps = React.ComponentProps<'div'> & {
  ...
  /** External control for the mobile sheet's open state, e.g. from a shell-level
   *  hamburger button that lives outside this provider's own trigger/rail. Mirrors
   *  the desktop open/onOpenChange pattern above -- Ariya's TopBar opens the mobile
   *  nav via AppContext's mobileNavOpen, not a control rendered inside the sidebar. */
  openMobile?: boolean;
...
function SidebarProvider({
  ...
  const setOpenMobile = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      const nextState = typeof value === 'function' ? value(openMobile) : value;
      if (setOpenMobileProp) {
        setOpenMobileProp(nextState);
      } else {
        _setOpenMobile(nextState);
      }
    },
...
function SidebarMenuButton({
  ...
  // Tooltip only makes sense when the label itself isn't visible on-screen
  // (collapsed rail, desktop). When expanded the label is already inline --
  // no need for a hover tooltip. Previously this rendered the Tooltip
  // unconditionally and passed `hidden` as a prop to TooltipContent hoping
  // it would gate visibility, but the underlying primitive spreads unknown
  // props straight onto a plain div (`hidden={false}` renders no attribute
  // at all), so the tooltip content -- forceMount'd, unportaled -- stayed
  // permanently visible in normal document flow for every nav item
  // (critique 2026-07-28: this is what actually produced the "label text
  // spilling out when collapsed" bug, not the outer label <span>).
