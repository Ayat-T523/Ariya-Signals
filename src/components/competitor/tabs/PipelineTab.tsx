...
const MINI_COLS = [
  ...
  { label: '2028',  year: 2028, q: null },
  { label: '2029',  year: 2029, q: null },
]
...
const MINI_ROW_H    = 42   // data row height
...
function findColIdx(year, q) {
  ...
  if (year === 2029) return 11
  if (year === 2028) return 10
...
function MilestoneIcon({ type }) {
  if (type === 'readout')  return <span style={{ fontSize: 10, color: '#F59E0B', lineHeight: 1 }}>●</span>
  if (type === 'filing')   return <FileText size={10} color="#0055BB" />
  if (type === 'approval') return <Check size={10} color="#059669" strokeWidth={2.5} />
  return <span style={{ fontSize: 12, color: 'rgba(5,10,68,0.30)', lineHeight: 1 }}>○</span>
}
...
function MiniGantt({ indicationSubtype, currentCompetitorId }) {
  ...
  const rows = COMP_ROWS_BY_TYPE[type]
    ...
    <div style={{ overflowX: 'auto' }}>
      <div style={{ minWidth: totalW }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '0 0 8px', flexWrap: 'wrap' }}>
          {[
            { marker: <span style={{ fontSize: 10, color: '#F59E0B' }}>●</span>,                                                                                 label: 'Data readout'      },
            { marker: <FileText size={10} color="#0055BB" />,                                                                                                     label: 'Filing'            },
            { marker: <Check size={10} color="#059669" strokeWidth={2.5} />,                                                                                      label: 'Approval'          },
            { marker: <span style={{ fontSize: 12, color: 'rgba(5,10,68,0.30)', lineHeight: 1 }}>○</span>,                                                        label: 'Phase start'       },
            { marker: <span style={{ display: 'inline-block', width: 18, height: 7, borderRadius: 2, background: MINI_PHASE_CFG.own.bg, border: `1px solid ${MINI_PHASE_CFG.own.border}` }} />, label: 'Ekterly (own product)' },
            { marker: <span style={{ display: 'inline-block', width: 18, height: 7, borderRadius: 2, background: 'rgba(245,158,11,0.18)' }} />,                  label: 'Today'             },
          ].map((item, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
              {item.marker}
              <span style={{ fontSize: 11, color: 'rgba(5,10,68,0.50)' }}>{item.label}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{
          ...
          <div style={{ width: MINI_LABEL_W, flexShrink: 0, padding: '6px 10px', borderRight: '1px solid rgba(5,10,68,0.08)' }}>
            <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.09em', color: 'rgba(5,10,68,0.40)' }}>Competitor</span>
          </div>
          <div style={{ width: MINI_THREAT_W, flexShrink: 0, padding: '6px 4px', borderRight: '1px solid rgba(5,10,68,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.09em', color: 'rgba(5,10,68,0.40)' }}>Threat</span>
          </div>
          {MINI_COLS.map((col, i) => (
            <div key={i} style={{
              ...
              <span style={{ fontSize: 10, fontWeight: col.isCurrent ? 700 : 500, color: col.isCurrent ? '#C01041' : 'rgba(5,10,68,0.38)' }}>
                {col.label}
              </span>
            </div>
          ))}
        ...
        <div style={{ padding: '5px 10px', background: 'rgba(5,10,68,0.02)', borderBottom: '1px solid rgba(5,10,68,0.06)' }}>
          <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.09em', color: 'rgba(5,10,68,0.38)' }}>
            {sectionLabel}
          </span>
        </div>
          ...
          return (
            <div key={ri} style={{
              ...
              <div style={{
                ...
                <span style={{ fontSize: 11, fontWeight: 700, color: 'rgba(5,10,68,0.80)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {row.name}
                </span>
                <span style={{ fontSize: 10, color: 'rgba(5,10,68,0.40)', fontStyle: 'italic', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {row.drugLabel}
                </span>
              </div>
              ...
              <div style={{
                ...
                {row.isOwn ? (
                  <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 5px', borderRadius: '9999px', background: 'rgba(236,72,153,0.10)', color: '#BE185D' }}>Own</span>
                ) : threatCfg ? (
                  <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 5px', borderRadius: '9999px', background: threatCfg.bg, color: threatCfg.text }}>
                    {row.threat}
                  </span>
                ) : null}
              </div>
              ...
              {row.milestones.map((m, mi) => (
                <div key={mi} style={{
                  ...
                  <span style={{ fontSize: 8, color: 'rgba(5,10,68,0.45)', whiteSpace: 'nowrap' }}>{m.label}</span>
                </div>
              ))}
            </div>
          )
...
// ── Clinical milestones & trial design (collapsible) ─────────────────────────
function ClinicalDetailsSection({ asset }) {
    ...
    <div style={{ background: 'rgba(5,10,68,0.02)', borderRadius: '8px', overflow: 'hidden' }}>
      <button
        ...
        style={{
          ...
          fontSize: '13px', fontWeight: 600, color: 'rgba(5,10,68,0.65)',
          fontFamily: 'inherit',
        }}
      >
        <span>Clinical milestones &amp; trial design</span>
...
function AssetCard({ asset, competitorId }) {
  ...
  return (
    <div style={{
      ...
      {/* Clinical milestones & trial design */}
      <ClinicalDetailsSection asset={asset} />
