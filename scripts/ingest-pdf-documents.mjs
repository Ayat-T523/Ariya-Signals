...
if (!SUPABASE_URL || !SERVICE_KEY) {
  ...
  console.error('   Run with: node --env-file=.env.local scripts/ingest-pdf-documents.mjs')
  process.exit(1)
}
...
const DOCUMENTS = [
  {
    ...
    datePublished: '2025-08-01',  // FY2025 report (CSL fiscal year ends 30 June)
    description:  'CSL Limited FY2025 Annual Report — ASX filer; includes CSL Behring pipeline, Andembry commercial update, and group financials',
  },
  {
...
for (const doc of DOCUMENTS) {
  ...
  if (wordCount < 100) {
    errors.push(`${doc.localPath}: too little text extracted (${wordCount} words)`)
    console.log(`⚠️  only ${wordCount} words extracted — PDF may be image-based`)
    continue
  }
